...
// @version 3.54
//k==/UserScript==
/************VariablenprA?fung und Initialisierung ************************/
checkgm_vars()
...
<h3 id="menu7" tabindex="-1" aria-selected="false" aria-expanded="false" role="tab" class="ui-accordion-header ui-helper-reset ui-state-default ui-corner-all"><span class="ui-icon ui-icon-triangle-1-e"></span><a tabindex="-1" href="#">'+'Send Troops'+'</a></h3>\
<table id="jonny_VillagesToAttackTable" style="display: none; overflow: visible; padding-top: 0px; padding-bottom: 0px;" role="tabpanel" class="ui-accordion-content ui-helper-reset ui-widget-content ui-corner-bottom">\
<tbody>\
<tr>\
<td><h5>Farmer</h5><input id="jonny_Farmer" style="width:33px" type="text"></td>\
<td><h5>Sword</h5><input id="jonny_Sword" style="width:33px" type="text"></td>\
<td><h5>Spear</h5><input id="jonny_Spear" style="width:33px" type="text"></td>\
<td><h5>Axe</h5><input id="jonny_Axe" style="width:33px" type="text"></td>\
<td><h5>Bow</h5><input id="jonny_Bow" style="width:33px" type="text"></td>\
<td><h5>Spy</h5><input id="jonny_Spy" style="width:33px" type="text"></td>\
<td><h5>Light</h5><input id="jonny_Light" style="width:33px" type="text"></td>\
<td><h5>Heavy</h5><input id="jonny_Heavy" style="width:33px" type="text"></td>\
<td><h5>Ram</h5><input id="jonny_Ram" style="width:33px" type="text"></td>\
<td><h5>Kata</h5><input id="jonny_Kata" style="width:33px" type="text"></td>\
<td><h5>Snob</h5><input id="jonny_Snob" style="width:33px" type="text"></td>\
</tr>\
<tr>\
<td colspan="6"> Interval </td>\
</tr>\
<tr>\
<td>hh:<input id="jonny_Hours" style="width:33px" type="text"></td>\
<td>mm:<input id="jonny_Mins" style="width:33px" type="text"></td>\
<td>ss:<input id="jonny_Secs" style="width:33px" type="text"></td>\
</tr>\
<tr><td colspan="6">Target villages:</td></tr>\
<tr>\
<td colspan="3"><textarea id="jonny_Villages_Coords" rows="5"style="resize:none; border: 1px solid #c4a76e; background-color: #f6dba4;" ></textarea></td>\
</tr>\
<tr>\
<td colspan="11"><progress id="jonny_Progress_Attack" max="100" style="width:380px"</td>\
</tr>\
<tr>\
<td colspan="11"> <button id="attackButton" class="configbutton" >Attack</button> </td>\
</tr>\
</tbody>\
</table>\
</div>\
...
ID("attackButton").addEventListener('click',function() {sendAttack();},false)


var villages;
function sendAttack(){
    villages = ID("jonny_Villages_Coords").value.split('\n');
	sendSubAttack(0);

}
function sendSubAttack(currentVillage){

	var targetX = villages[currentVillage].split('|')[0];
	var targetY = villages[currentVillage].split('|')[1];
	
 
	
	var farmer = ID("jonny_Farmer").value;
	var sword = ID("jonny_Sword").value;
	var spear = ID("jonny_Spear").value;
	var axe = ID("jonny_Axe").value;
	var bow = ID("jonny_Bow").value;
	var spy = ID("jonny_Spy").value;
	var light = ID("jonny_Light").value;
	var heavy = ID("jonny_Heavy").value;
	var ram = ID("jonny_Ram").value;
	var kata = ID("jonny_Kata").value;
	var snob = ID("jonny_Snob").value;
	
	var hh = ID("jonny_Hours").value;
	var mm = ID("jonny_Mins").value;
	var ss = ID("jonny_Secs").value;
	
	var interval = hh * 3600 + mm * 60 + ss;
	
	var regex= /village=(\d+)*/;
    regex.exec(document.location.href);
    var vilnum=RegExp.$1;
    
    ID("jonny_Progress_Attack").value = (currentVillage / villages.length) * 100;
    
	if (targetX==undefined || targetY==undefined || isNaN(targetX) || isNaN(targetY)) {
    	//alert("Incorrect input");
    	nextVillage(currentVillage, 0);
    }
    else {
    	GM_xmlhttpRequest(
    	{
    		method: "POST",
    		url: "http://s1.kingsage.ru/game.php?village="+vilnum+"&s=build_barracks&m=command&sub=send",
    		headers: { 'User-agent': 'Mozilla/5.0 (compatible) Greasemonkey',
    					'Accept': 'text/plain',
    					'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8',
    					'Accept-Charset': 'ISO-8859-1,utf-8;q=0.7,*;q=0.7' },
    		data: encodeURI("farmer="+farmer+"&sword="+sword+"&spear="+spear+"&axe="+axe+"&bow="+bow+"&spy="+spy+"&light="+light+"&heavy="+heavy+"&ram="+ram+"&kata="+kata+"&snob="+snob+"&send_x="+targetX+"&send_y="+targetY+"&attack=%D0%90%D1%82%D0%B0%D0%BA%D0%B0&espy=&support=&discharge="),
    		onload: function (responseDetails) {
            	GM_xmlhttpRequest(
            	{
            		...
            		onload: function (responseDetails) {
    					...
    					//alert("attack to village " + targetX + ":" + targetY);
    					GM_xmlhttpRequest(
    					{
    						method: "POST",
    						url: "http://s1.kingsage.ru/game.php?village="+vilnum+"&s=build_barracks&m=command&a=sendTroop&p="+pcode,
    						headers: { 'User-agent': 'Mozilla/5.0 (compatible) Greasemonkey',
    									'Accept': 'text/plain',
    									'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8',
    									'Accept-Charset': 'ISO-8859-1,utf-8;q=0.7,*;q=0.7' },
    						data: encodeURI("farmer="+farmer+"&sword="+sword+"&spear="+spear+"&axe="+axe+"&bow="+bow+"&spy="+spy+"&light="+light+"&heavy="+heavy+"&ram="+ram+"&kata="+kata+"&snob="+snob+"&send_x="+targetX+"&send_y="+targetY+"&attack=%D0%90%D1%82%D0%B0%D0%BA%D0%B0&espy=&overwrite_attack=1"),
    					});
    				}
    	...
    	nextVillage(currentVillage, interval);
    }

}	

function nextVillage(currentVillage, interval) {
	if(currentVillage < villages.length - 1) {
		setTimeout(sendSubAttack, interval * 1000, currentVillage + 1);
	}	   
}
...
ID("menu7").addEventListener('click',function() {GM_setValue('lastmenutab',7);},false)
...
for(var i=1;i<9;i++) { /**** *FA?r Extrabuttons*/
...
//PrA?fen der Seite und aufrufen der Extrabuttons-Funktion
if (GM_getValue('extrabuttons')) {
if (GM_getValue("extrabuttonstab")) {
tab=true
} else {
...
{
/* EinfA?gen des Formatierungscodes */
...
t=t.replace(/\r\n/img,"<br>") // zeilen umbrA?che
t=t.replace(/\n\r/img,"<br>")// zeilen umbrA?che
t=t.replace(/\n/img,"<br>")// zeilen umbrA?che
t=t.replace(/\r/img,"<br>")// zeilen umbrA?che
...
str+="<div class='message_number'>"+string["tit_message"]+"&nbsp;"+(i+1)+"</div>" //hier noch A¤ndern
str+=messages[i]
}
...
str+="<div class='message_number'>"+string["tit_report"]+"&nbsp;"+(i+1)+"</div>" //hier noch A¤ndern
str+=reports[i]
}
...
//LA¶schen der Nachrichten/Berichte
...
ind1=lref().indexOf("info") //adden zur A?bersicht...
ind2=lref().indexOf("=")
...
//gibt ID(elem) zurA?ck
function ID(obj) {
id=document.getElementById(obj);
return id;
}
//gibt location.href zurA?ck
function lref() {
...
info: parse(str,"messagesaver_info","Save messages & reports with one click. Read and delete them in the Kingsage aˆzToolsaˆ?.")
},
villageinfo: {
...
dest: parse(str,"calc_dest","<img src='http://www.kingsage.de/img/arrow_right_raquo.png'><img src='http://www.kingsage.de/img/arrow_right_raquo.png'> A?berblick <img src='http://www.kingsage.de/img/arrow_right_raquo.png'> Production, Troops"),
...
info:parse(str,"worldmap_info","The aˆzGenerateaˆ?-Button creates a worldmap from mini-maps. Important: Map settings - 9 x 9 sections, 5 pixel.")
},
multimarket: {
